...
export default function Products() {
  ...
  const [filters, setFilters] = useState({ category: '', sort: 'createdAt:desc', search: '', maxPrice: '' });

  // Sync URL -> filters
  useEffect(() => {
    if (!router.isReady) return;
    const { category = '', sort = 'createdAt:desc', search = '', maxPrice = '' } = router.query;
    setFilters({ category, sort, search, maxPrice });
    setPage(1);
  }, [router.isReady, router.query]);

  // Load categories
  useEffect(() => {
    productsAPI.getAll({ limit: 1 }).then(r => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (filters.maxPrice) params.maxPrice = parseFloat(filters.maxPrice) / (83 * 1.45);
    ...
    <>
      ...
      <div className="bg-background-light min-h-screen text-slate-900" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          {/* Header */}
          <div className="mb-10">
            <h1 className="text-4xl font-extrabold tracking-tight mb-3">{filters.category || 'All Products'}</h1>
            <p className="text-slate-500 text-sm">{total} products found</p>
          </div>
            ...
            <aside className="w-60 shrink-0 hidden lg:block">
              <div className="sticky top-28 space-y-8">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input type="radio" name="cat" checked={!filters.category} onChange={() => updateFilter('category', '')} className="text-primary" />
                      <span className="text-sm text-slate-600 group-hover:text-primary transition-colors">All</span>
                    </label>
                    {categories.slice(0, 10).map(c => (
                      <label key={c} className="flex items-center gap-3 cursor-pointer group">
                        <input type="radio" name="cat" checked={filters.category === c} onChange={() => updateFilter('category', c)} className="text-primary" />
                  ...
                  <div className="space-y-2">
                    {[['createdAt:desc', 'Newest'], ['price:asc', 'Price: Low to High'], ['price:desc', 'Price: High to Low'], ['name:asc', 'Name A-Z']].map(([val, label]) => (
                      <label key={val} className="flex items-center gap-3 cursor-pointer group">
                        <input type="radio" name="sort" checked={filters.sort === val} onChange={() => updateFilter('sort', val)} className="text-primary" />
                ...
                <div>
                  ...
                  <input
                    type="number" placeholder="e.g. 5000" step="500"
                    value={filters.maxPrice ? Math.round(parseFloat(filters.maxPrice)) : ''}
